    ...
    const solveWaterJug = (x, y, z) => {
        ...
        while (queue.length > 0) {
            ...
            const actions = [
                ...
                { a: Math.max(0, a - (y - b)), b: Math.min(y, a + b), action: `Transfer jug A into B (A: ${Math.max(0, a - (y - b))}, B: ${Math.min(y, a + b)})` },
                { a: Math.min(x, a + b), b: Math.max(0, b - (x - a)), action: `Transfer jug B into A (A: ${Math.min(x, a + b)}, B: ${Math.max(0, b - (x - a))})` }
            ];
